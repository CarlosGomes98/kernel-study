...
__global__ void kernel(float *A, float *B, float *O, int H, int W, int K)
{
    ...
    for (int k = 0; k < K; k += BLOCK_SIZE) {
        ...
        tileB[threadIdx.y * BLOCK_SIZE + threadIdx.x] = B[(threadIdx.y + k) * K + xIndex];
...
int main(void)
{
    ...
    static_assert(SIZE % BLOCK_SIZE == 0, "SIZE must be multiple of BLOCK_SIZE");
    dim3 block((int) BLOCK_SIZE, (int) BLOCK_SIZE, 1); // improve this
